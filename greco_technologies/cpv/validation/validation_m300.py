...
columnnames = pd.read_csv(
    "../inputs/m300_measurements_headers.csv", sep=",", dtype={"Daytime": str}
)
...
celltemp = csys.pvsyst_celltemp(df["GNI"], df["AirTemperature"], df["WindSpeedAvg"])
...
for i, v in relative_airmass.items():
    uf_am.append(cpv.get_simple_util_factor(v, thld_am, m_low_am, m_high_am))
...
for i, v in df["AirTemperature"].items():
    uf_temp.append(cpv.get_simple_util_factor(v, thld_temp, m_low_temp, m_high_temp))
...
plt.plot(
    ...
    "r+",
    markersize=1,
    label="Temperature residual with UF",
)
