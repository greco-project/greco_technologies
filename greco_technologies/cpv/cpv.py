...
def create_cpv_timeseries(lat, lon, weather, surface_azimuth, surface_tilt,
                          type, calc_uf_aoi=True):

    if type == 'ins':
        module_params = inputs.create_ins_dict()
    ...
    for i, v in relative_airmass.items():
        uf_am.append(cpv.get_simple_util_factor(v, module_params['thld_am'],
                                                module_params['m_low_am'] /
                                                module_params['IscDNI_top'],
                                                module_params['m_high_am'] /
                                                module_params['IscDNI_top']))
    uf_temp = []
    for i, v in weather['temp_air'].items():
        uf_temp.append(cpv.get_simple_util_factor(v,
                                                  module_params['thld_temp'],
                                                  module_params['m_low_temp'] /
                                                  module_params['IscDNI_top'],
                                                  module_params['m_high_temp']
                                                / module_params['IscDNI_top']))

    uf_am_temp = np.multiply(module_params['weight_am'], uf_am) + \
                 np.multiply(module_params['weight_temp'], uf_temp)
    if calc_uf_aoi==True:
        uf_aoi = []
        for i, v in weather['aoi'].items():
            uf_aoi.append(
                cpv.get_simple_util_factor(v, module_params['thld_aoi'],
                                           module_params['m_low_aoi'] /
                                           module_params['IscDNI_top'],
                                       module_params['m_high_aoi'] /
                                           module_params['IscDNI_top']))

        uf_aoi_top = cpv.get_simple_util_factor(0, module_params['thld_aoi'],
                                            module_params['m_low_aoi'] /
                                                module_params['IscDNI_top'],
                                            module_params['m_high_aoi'] /
                                                module_params['IscDNI_top'])
...
if __name__ == '__main__':
    filename = os.path.abspath(
        '/home/local/RL-INSTITUT/inia.steinbach/Dokumente/greco-project/pvcompare/pvcompare/Data/weatherdata.csv')
    ...
    ds=create_cpv_timeseries(lat=40.3, lon=5.4, weather=weather_df,
